...
      aria-label="روبوت فزعة" role="button" tabIndex={0}
      onClick={onClick}
      onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onClick?.(); }}
    >
      <img src="/assets/fazaa-robot-transparent.png" alt="روبوت فزعة" className="h-full w-full object-cover transition-transform duration-150" style={{ transform: `translate(${lookX}px, ${lookY}px) scale(1.03)` }} />
      {mood === "blink" && <>
...
      let botText = "";
      if (type === 'quick') {
        botText = "🤖 فزعة: دزيت فزعة سريعة! جاي أدور 3 لاعبين قريبين من البصرة أونلاين.. ⏳";
        // هنا تسوي insert بجدول fazaa_requests لاحقاً
        // await supabase.from('fazaa_requests').insert({ requester_id: userId, match_id: matchId, type: 'quick' })
      }
      if (type === 'balance') botText = "🤖 فزعة: تمام، جاي أوازن الفرق حسب المستويات.. ⚖️";
      if (type === 'keeper') botText = "🤖 فزعة: أدورلك حارس مرتب قريب.. 🧤";

      // رسالة البوت تنحفظ كرسالة عادية لكن بمعرف البوت
...
  useEffect(() => {
    supabase?.auth.getSession().then(({data})=>{ const id = data?.session?.user?.id; if(id && id!== 'undefined' && id!== 'null') setUserId(id); });
...
      {/* قائمة أوامر فزعة - تطلع من الهيدر */}
      {fazaaMenuOpen && (
...
                    <div className={`relative inline-block w-fit max-w-[78%] cursor-pointer rounded- px-4 py-3 text-sm leading-relaxed shadow-[0_2px_8px_rgba(0,0,0,0.06)] whitespace-pre-wrap break-words transition-shadow ${isFazaa?"rounded-bl- bg-[#0f172a] border-2 border-[#39ff88] text-white": isMine?"rounded-br- bg-[#00d978] text-[#032015]":"rounded-bl- border border-slate-100 bg-white text-slate-800 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"} ${selectedMessageId === (m.id || i)? "ring-2 ring-[#39ff88] ring-offset-2 ring-offset-[#0b1215]" : ""}`} onClick={() => setSelectedMessageId(selectedMessageId === (m.id || i)? null : (m.id || i))} style={{ overflowWrap: 'anywhere' }}>
