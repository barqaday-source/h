...
export async function fetchHomeData({ query = "" } = {}) {
  ...
  let storiesData = [];
  try {
    const { data, error } = await client.from("stories").select("id,title,image_url,media_url,user_id,created_at").order("created_at", { ascending: false }).limit(30);
    if (error) throw error;
    storiesData = data;
  } catch {
    const { data } = await client.from("stories").select("id,title,image_url,created_at").order("created_at", { ascending: false }).limit(12);
    storiesData = (data || []).map(s => ({ ...s, user_id: s.user_id || "unknown", media_url: s.image_url }));
  }
  const [matchesResult, pinsResult] = await Promise.all([
    client.from("matches").select("id,format,starts_at,max_players,venue:venues(id,name,latitude,longitude),participants:match_participants(count)").eq("status", "open").order("starts_at", { ascending: true }).limit(20),
    client.from("venues").select("id,name,address,latitude,longitude").not("latitude", "is", null).not("longitude", "is", null).limit(100),
  ]);
  ...
  const matches = (matchesResult.data ?? []).map((match) => {
    ...
    return {
      ...match,
      pitch: match.venue?.name ?? "ملعب غير مسمى",
...
export async function fetchCurrentMatchId() {
  ...
  const client = requireSupabase();
  const { data, error } = await client.from("matches").select("id").eq("status", "open").order("starts_at", { ascending: true }).limit(1).maybeSingle();
  if (error) throw error;
  return data?.id ?? GENERAL_MATCH_ID;
}
...
export async function fetchNotifications() {
  ...
  const { data, error } = await client.from("notifications").select("id,kind,title,body,match_id,invitation_id,read_at,created_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(20);
  if (error) throw error;
  return data ?? [];
}
...
export async function respondToInvitation(invitationId, status) {
  ...
  const { data, error } = await client.from("match_invitations").update({ status }).eq("id", invitationId).eq("invitee_id", userId).select("id,match_id,status").single();
  if (error) throw error;
  if (status === "accepted") {
    const { error: participantError } = await client.from("match_participants").upsert({ match_id: data.match_id, user_id: userId, status: "confirmed" }, { onConflict: "match_id,user_id" });
...
export async function markNotificationRead(notificationId) {
  ...
  const { error } = await client.from("notifications").update({ read_at: new Date().toISOString() }).eq("id", notificationId).eq("user_id", userId);
  if (error) throw error;
}
...
export async function fetchFazaaData() {
  ...
  const [requestsResult, playersResult, campaignResult] = await Promise.all([
    client.from("fazaa_requests").select("id,need,starts_at,match_id,match:matches(venue:venues(name)),requester:profiles(id,display_name,avatar_url)").eq("status", "open").order("starts_at", { ascending: true }).limit(20),
    client.from("profiles").select("id,display_name,avatar_url,is_online,latitude,longitude,role,rating").eq("is_available", true).order("last_seen_at", { ascending: false }).limit(20),
    client.from("campaigns").select("id,title,subtitle,raised_amount,goal_amount").eq("status", "active").order("created_at", { ascending: false }).limit(1).maybeSingle(),
  ]);
...
export async function fetchProfile(userId) {
  ...
  const [profileResult, statsResult, badgesResult, gamesResult] = await Promise.all([
    client.from("profiles").select("id,display_name,city,role,status,rating,avatar_url,allow_jawk_requests,is_available,is_online,last_seen_at").eq("id", userId).maybeSingle(),
    client.from("player_stats").select("games,wins,goals").eq("user_id", userId).maybeSingle(),
    client.from("player_badges").select("id,label").eq("user_id", userId).order("created_at", { ascending: false }),
    client.from("recent_games").select("id,pitch,rating,played_at").eq("user_id", userId).order("played_at", { ascending: false }).limit(10),
  ]);
...
export async function fetchMessages(matchId) {
  const client = requireSupabase();
  const realId = !matchId || matchId === "general" ? GENERAL_MATCH_ID : matchId;
  try {
    const { data, error } = await client
      .from("messages")
      .select("id,body,message_type,attachment_url,attachment_name,created_at,user_id,author:profiles(display_name,avatar_url)")
      .eq("match_id", realId)
    ...
    return (data ?? []).map((message) => ({
      ...
      time: new Date(message.created_at).toLocaleTimeString("ar-IQ", { hour: "numeric", minute: "2-digit" }),
      mine: message.user_id === userId,
      user_id: message.user_id,
    }));
  } catch (err) {
    console.error("fetchMessages failed:", err.message);
    return [];
  }
}
...
export async function donate({ campaignId, amount }) {
  ...
  const { error } = await client.from("campaign_donations").insert({ campaign_id: campaignId, user_id: userId, amount });
  if (error) throw error;
}
...
export async function viewStory(storyId) {
  ...
  try {
    ...
    await client.from("story_views").upsert({ story_id: storyId, viewer_id: viewerId }, { onConflict: "story_id,viewer_id" });
  } catch (e) {
    console.log("viewStory error:", e.message);
  }
}
...
export async function fetchStoryViewers(storyId) {
  try {
    const client = requireSupabase();
    const { data, error } = await client.from("story_views").select(`created_at, viewer_id, profiles:viewer_id ( id, display_name, full_name, avatar_url )`).eq("story_id", storyId).order("created_at", { ascending: false });
    if (error) throw error;
    return data ?? [];
  } catch (e) {
    return [];
